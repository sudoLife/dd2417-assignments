...
class CKY:
    ...
    def parse(self, s):
        ...
        for j in range(length):
            # unary rules
            self.table.append([[]] * length)  # create the cells
            # create the corresponding backptr row
            self.backptr.append([[]] * length)
            self.table[j][j] = self.unary_rules[self.words[j]]

            for i in range(j - 1, -1, -1):
                start_word = i
                stop_word = j + 1  # not including this word

                results = []
                backptrs = []
                # now we need all words that make this range
                for k in range(start_word + 1, stop_word):
                    # just for clarity
                    first_cell = self.table[start_word][k - 1]
                    second_cell = self.table[k][j]

                    combo = [[x, y] for x in first_cell for y in second_cell]

                    for pair in combo:
                        try:
                            result = self.binary_rules[pair[0]][pair[1]]
                            for res in result:
                                backptrs.append((
                                    res, pair, k))
                                results.append(res)
                        # nobody cares about errors, keep going
                        except KeyError:
                            pass
                self.table[i][j] = results
                self.backptr[i][j] = backptrs
    ...
    def print_trees(self, row, column, symbol, top=True):
        ...
        for sym, pair, k in self.backptr[row][column]:
            if sym == symbol:  # one of our guys
                # [row, k - 1] and [k, col]
                print(f'{sym}(', end='')
                self.print_trees(row, k - 1, pair[0], False)
                print(', ', end='')
                self.print_trees(k, column, pair[1], False)
                print(')', end='')
                # newline before a new tree
                if top:
                    print("")
